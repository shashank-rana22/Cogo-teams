...
function CallAnalytics({ loading = true, callsAnalytics }) {
							...
							<div className={styles.time_durations}>
								{loading
									...
									: (
										...
										>
											<span className={styles.time_durations_value}>{(callsAnalytics?.[itemKey] || 0) >= 60 ? ((callsAnalytics[itemKey] || 0) / 60).toFixed(2) : (callsAnalytics[itemKey] || 0)}</span>
											{' '}
									...
									{loading
										...
										: (
											<div className={styles.customer_nos}>
												<span>{callsAnalytics?.[stat.key]}</span>
												{' '}
