...
const useListLeadOrganization = ({ search = '' }) => {
	...
	const leadOrganizations = useCallback(async ({ page }) => {
		try {
	...
	const handleScroll = (e) => {
		...
		if (reachBottom && !loading && !listData?.isLastPage) {
			leadOrganizations({ page: pagination + NEXT_PAGE_COUNT });
		}
	};
	...
	useEffect(() => {
		...
		leadOrganizations({ page: DEFAULT_PAGE_NUMBER });
	}, [leadOrganizations]);
...
export default useListLeadOrganization;
