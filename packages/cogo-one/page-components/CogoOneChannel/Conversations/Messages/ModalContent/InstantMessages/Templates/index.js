import { cl, Toast, Input, Button, Placeholder, Pill } from '@cogoport/components';
	...
	const handleSelect = (val, status) => {
		if (!openCreateReply) {
			if (status === 'approved' || openCreateReply) {
				...
				Toast.error('Template is not verified');
			}
		}
	};
			...
			<div className={styles.messages_container}>
				<div className={styles.container}>
					...
					>
								...
								<div className={styles.wrap}>
									...
									<div>
										{whatsapp_approval_status === 'approved' && (
											<Pill size="md" color="green">Approved</Pill>)}
										{whatsapp_approval_status === 'rejected' && (
											<Pill size="md" color="red">Rejected</Pill>)}
										{whatsapp_approval_status === null && (
											<Pill size="md" color="orange">Pending</Pill>)}
									</div>
								</div>
						...
						{loading && loader()}
						{isEmpty(list) && !loading && <div className={styles.empty_div}>No Templates Found</div>}
					</div>
				</div>
