...
const getCollectionPartyParams = ({ profile_data, servicesData = [] }) => {
	...
	const isImport = getTradeTypeByIncoTerm(primary_service?.inco_term);
	...
	if (isFcl && isImport) {
		...
		if (user_role_ids.includes((i) => [...geo.uuid.service_ops2_role_id, geo.uuid.so_2_manager].includes(i))) {
