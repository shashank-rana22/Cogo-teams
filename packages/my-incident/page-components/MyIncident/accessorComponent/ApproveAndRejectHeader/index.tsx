...
function ApproveAndRejectHeader({ row = {} }: Props) {
	...
	return (
		<div
			className={
				status === 'APPROVED'
					? styles.approve_container
					: styles.reject_container
			}
		>
			<div className={styles.flex}>
				<div className={styles.remark_flex}>
					<div className={styles.icon}>
						{status === 'APPROVED' ? (
							<IcCFtick width="22px" height="22px" />
						) : (
			...
			<div
				className={
					status === 'APPROVED' ? styles.hr : styles.rejected_hr
				}
			/>
			<div
				className={
					status === 'APPROVED'
						? styles.remarks
						: styles.rejected_remarks
				}
			>
				<div>Remarks :</div>

				<div style={{ marginLeft: '8px' }}>{remark}</div>
			</div>
		</div>
	);
}
