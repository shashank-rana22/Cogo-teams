...
const useFieldMutation = ({
	...
	const newfields = fields.map((control) => {
		...
		if (type === 'fieldArray' && name.includes('line_items')) {
			const chargeCodes =	name === 'fcl_customs_cfs_line_items'
				? chargeCfsCode
				: genericLineitems;
			const codeMapping = data?.[control.charge_code_name || 'freight_charge_codes'] || {};
			...
			newControl.controls = newControl.controls.map((childCtrl) => {
				...
				if (childCtrl.name === 'unit') {
					...
					(chargeCodes || []).forEach((item, i) => {
						UNIT_OPTIONS[i] = (
							codeMapping?.[item.code]?.units || ['per_container']
						).map((unit) => ({
