...
const ImportRoles = ({ show, onClose, onSubmit = () => {} }) => {
	...
	options.forEach((role) => {
		...
		(role.permissions || []).forEach((permission) => {
			hashedRole[`${permission.navigation}:${permission.resource_name}`] =
				permission;
		});
	...
	const handleSubmit = () => {
		if (view === 'import') {
			if (options.length > 1) {
				...
				const permissions =
					options.find((role) => role.id === firstRoleId)?.permissions || [];
			...
			<div style={{ padding: '22px' }}>
				...
				{view === 'import' ? (
					<Row>
						<SelectBox className="full">
							...
							<Select
								...
								selectType="pills"
								onChange={(val) =>
									setFormvalues({ ...formValues, partner_id: val })
								}
								isClearable
							/>
						</SelectBox>
					...
					<Priority
						...
						options={options}
						setPriority={(ids) =>
							setFormvalues({ ...formValues, role_ids: ids })
						}
					/>
				)}
			</div>
