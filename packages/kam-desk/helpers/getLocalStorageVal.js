...
const getLocalStorageVal = () => {
	...
	if ((!stepperTab || !STEPPER_CONFIG[shipment_type]?.some((item) => item.value === stepperTab))
	&& shipment_type !== 'all') {
		stepperTab = STEPPER_CONFIG[shipment_type]?.[GLOBAL_CONSTANTS.zeroth_index]?.value;
	}

	const shipmentTypeTabs = ShipmentTabMapping?.[shipment_type]?.[stepperTab]?.tabs || [];

	if (!activeTab || !shipmentTypeTabs?.some((item) => item.value === activeTab)) {
