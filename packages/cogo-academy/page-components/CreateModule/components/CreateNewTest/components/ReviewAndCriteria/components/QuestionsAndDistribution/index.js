	...
	useEffect(() => {
		data?.test_set_distribution_data?.forEach((test_set_distribution) => {
			if (test_set_distribution.question_type === 'stand_alone') {
				...
				}q`, test_set_distribution.distribution_count);	// append with 'q' for stand alone questions
			} else {
				setValue(`${test_set_distribution.test_question_set_id
				// eslint-disable-next-line radix
				}c`, parseInt(test_set_distribution.distribution_count)); // append with 'c' for case study questions
			}
		});
