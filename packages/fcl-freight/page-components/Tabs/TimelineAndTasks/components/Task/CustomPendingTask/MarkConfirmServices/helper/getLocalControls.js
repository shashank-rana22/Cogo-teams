...
const getLocalControls = (service_type, shipment_data, formattedRate) => {
	const value = (shipment_data?.all_services || []).find((serviceObj) =>
		serviceObj?.service_type.includes('air_freight_local_service'),
	);
		...
		lcl_freight_service: [
			{
				...
				value: (shipment_data?.all_services || []).find((serviceObj) =>
					serviceObj?.service_type.includes('lcl_freight_local'),
				)?.service_provider_id,
				optionsListKey: 'verified-service-providers',
				placeholder: 'Select Service Provider',
				rules: { required: 'Service Provider is Required' },
