const getFunction = {
	condition: (field, formValues) =>
		(formValues.condition || []).includes(field.condition.condition),
	shipper_contact_status: (field, formValues) =>
		field.show_conditions.shipper_contact_status.includes(
...
const getShowElement = (field, formValues) => {
	...
	Object.keys(condition || {}).forEach((conditionName) => {
		flag =
			flag && (getFunction[conditionName] || (() => true))(field, formValues);
	});
