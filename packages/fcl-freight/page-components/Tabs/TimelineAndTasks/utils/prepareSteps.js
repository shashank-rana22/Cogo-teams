	...
	if (Array.isArray(value_to_insert)) {
		...
		(value_to_insert || []).forEach((valObj) => {
			...
			Object.keys(valObj || {}).forEach((key) => {
				newObj[key] =
					typeof valObj[key] === 'string' && valObj[key]?.includes('data')
...
const evaluateObject = (control, task, shipment_data) => {
	...
	if (control.conditions) {
		(control.conditions || []).forEach((obj) => {
			...
			if (!condition) {
				...
				const { leftHandSide, rightHandSide, value, elseValue } =
	...
	if (control?.type === 'fieldArray') {
		finalControl.controls = (control.controls || []).map((ctrl) => {
			return evaluateObject(ctrl, task, shipment_data);
		});
	}
...
const prepareSteps = (steps, task, primary_service = {}) => {
	...
	const dataRichUi = filteredSteps?.map((step) =>
		injectDataIntoValues(step, task, primary_service),
	);
