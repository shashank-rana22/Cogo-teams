...
const useEditLineItems = ({
			...
			if (control.type === 'edit_service_charges') {
				DEFAULT_VALUES[control.name] = control.value.map((value) => {
					...
					control.controls.forEach((subControl) => {
						FIELD_VALUE[subControl.name] = value[subControl.name] || ZERO_INDEX;
					});
	...
	const controls = services.map((service, index) => ({
		...
		value           : (service?.line_items || []).map((item) => ({
			...
			quantity         : item?.quantity || ZERO_INDEX,
			exchange_rate    : item?.exchange_rate || FIRST_INDEX,
			tax_percent      : item?.tax_percent || ZERO_INDEX,
			unit             : item?.unit,
			total            : item?.tax_total_price_discounted || ZERO_INDEX,
			name             : item?.name,
		...
		(Object.keys(formValues) || []).forEach((key) => {
			if (key && formValues?.[key]) {
				allFormValues[key] = (allFormValues[key] || []).map((value) => ({
					...
					total    : (value?.price_discounted || ZERO_INDEX) * (value?.quantity || ZERO_INDEX),
				}));
			}
		});
	...
	const LABELS = {};
	Object.keys(controls?.[ZERO_INDEX]).forEach((key) => {
		CUSTOM_VALUES[key] = {
