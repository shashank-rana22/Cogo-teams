...
const useUploadCargoArrivalForm = ({
	...
	const templateInitialValues = {
		...
		notify_party   : notify_party_details?.[GLOBAL_CONSTANTS.zeroth_index]?.company_name,
		...
		containers     : [
			{
				...(cargoArrivalData?.containers?.[GLOBAL_CONSTANTS.zeroth_index] || {}),
				marks_and_number : 'Nm',
		...
		shipment_details: {
			...
			vessel     : movement_details?.[GLOBAL_CONSTANTS.zeroth_index]?.vessel || '',
			voyage     : movement_details?.[GLOBAL_CONSTANTS.zeroth_index]?.voyage || '',
			obl_number : mbl_details?.[GLOBAL_CONSTANTS.zeroth_index]?.data?.document_number || '',
			hbl_no     : hbl_details?.[GLOBAL_CONSTANTS.zeroth_index]?.data?.document_number || '',
			obl_date   : formatDate({
				date       : mbl_details?.[GLOBAL_CONSTANTS.zeroth_index]?.updated_at,
