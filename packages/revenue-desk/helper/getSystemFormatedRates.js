const getSystemFormatedRates = (data, singleServiceData) => {
	...
	(data || []).forEach((element) => {
		...
		rowData.buy_price = Number(element?.validities?.[0]?.total_price) / Number(container_count);
		...
		rowData.total_buy_price = element?.validities?.[0]?.total_price || 0;
		rowData.total_buy_currency = element?.validities?.[0]?.currency;
		...
		rowData.validity_end = element?.validities?.[0]?.validity_end;
