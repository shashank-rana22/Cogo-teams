...
const getFormatedNotPreferenceData = ({ ratesDataNotPrefered, singleServiceData }) => {
	...
	(ratesDataNotPrefered || [])?.forEach((element) => {
		...
		const rowData = {
			...
			fulfillment_ratio_2      : element?.fulfillment_data?.fulfillment_ratio_2 || data?.fulfillment_ratio_2,
			fulfillment_ratio_7      : element?.fulfillment_data?.fulfillment_ratio_7 || data?.fulfillment_ratio_7,
			fulfillment_ratio_15     : element?.fulfillment_data?.fulfillment_ratio_15 || data?.fulfillment_ratio_15,
			fulfillment_ratio_30     : element?.fulfillment_data?.fulfillment_ratio_30 || data?.fulfillment_ratio_30,
