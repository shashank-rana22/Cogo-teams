...
const useCreateExpense = (nonRecurringData, setShowModal, getList, createExpenseType) => {
	...
	const branchId = JSON.parse(nonRecurringData?.branch || '{}')?.branchId;
	...
	useEffect(() => {
		if (addresses?.length > 0 && createExpenseType === 'nonRecurring') {
	...
	}, [branchId, addresses, createExpenseType]);
