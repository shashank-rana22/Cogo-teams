...
export const CardBody = ({ charge, type }: Props) => {
  ...
  const ToolTipContent = (quotation, actual) => (
    <div className={styles.content}>
      <div>
        <span className={styles.label}>Expected Quantity:</span> {quotation}
      </div>
      <div>
        <span className={styles.label}>Actual Quantity:</span> {actual}
      </div>
    </div>
  );
    ...
    >
      ...
      {(showFullDetails ? lineItems : lineItems.slice(0, 2)).map(
        (lineItem: GenericObject) => {
          const value =
            lineItem?.priceInInrActual - lineItem?.priceInInrQuotation;
