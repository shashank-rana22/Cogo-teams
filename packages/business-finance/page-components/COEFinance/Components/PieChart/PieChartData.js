	...
	return [
		{
			...
			value:
			(((bankDetailsRejectedCount || DEFAULT_VALUE) * PERCENTAGE_VALUE) / totalData).toFixed(DECIMAL_PLACE),
		},
		{
			id    : 'Billing',
			label : `Billing Party : ${billingPartyRejectCount || DEFAULT_VALUE}`,
			value : (
				((billingPartyRejectCount || DEFAULT_VALUE) * PERCENTAGE_VALUE) / totalData).toFixed(DECIMAL_PLACE),
		},
		{
			id    : 'Invoice',
			label : `Invoice Details :  ${invoiceDetailsRejectCount || DEFAULT_VALUE}`,
			value : (
				((invoiceDetailsRejectCount || DEFAULT_VALUE) * PERCENTAGE_VALUE) / totalData).toFixed(DECIMAL_PLACE),
		},
		{
			id    : 'Wrong',
			label : `Wrong Line Item : ${wrongLineItems || DEFAULT_VALUE}`,
			value : (((wrongLineItems || DEFAULT_VALUE) * PERCENTAGE_VALUE) / totalData).toFixed(DECIMAL_PLACE),
		},
	];
};
