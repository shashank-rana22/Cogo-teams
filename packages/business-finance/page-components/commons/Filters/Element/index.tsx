...
interface ElementProps{
	...
	filters:object;
	[key:string]:string|undefined|((val: any) => void)|Options[]|object|CSSProperties;
}
	...
	const getElement = () => {
		switch (type) {
			...
			case 'pdfView':
				return (
					<div>
						<div className={styles.urlContainer} style={style as CSSProperties} onClick={() => window.open(url, '_blank')}>
			...
			case 'pills':
				return (
					<>
					{rest?.options?.map((val)=>(<Pill size="sm" color="yellow" className={className} style={style as CSSProperties} >{val.label}</Pill>))};
					</>
				)
			case 'select':
				return (
					<div className={styles.select_container} style={{'--width':selectWidth||'200px'} as CSSProperties }>
					<Select value={value as string} className={className} options={options||[]} style={style as CSSProperties} {...rest}/>
					</div>
				);
			case 'input':
				return <Input value={value as string} style={style as CSSProperties} className={className} prefix={(
					<IcMSearchlight
						height={15}
						width={15}
					/>
			...
			case 'datepicker':
				return (
					<Datepicker name="date" value={value as Date} style={style as CSSProperties} {...rest} />
