...
const getParameters = ({ parameters = [] }) => parameters.filter((item) => item.status === 'active')
	?.map((parameter) => {
		const {
...
const useBlockWiseScoring = ({ data = {} }) => {
	...
	const prefillValues = useMemo(() => Object.entries(data.config_blocks?.filter((item) => item.status === 'active')
		?.reduce((acc, item) => {
